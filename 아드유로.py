...
@client.event
async def on_ready():
    await client.change_presence(status=discord.Status.online, activity=discord.Game("ad도움&ad광산도움(Beta)"))
    print("준비 되었다")
...
async def 룰렛(ctx):
    ...
    if result is None:
        ...
        if num1 == num2 == num3:
            ...
            val = (coin + 10, ctx.author.id)
            cursor.execute(sql, val)
            db.commit()
            await ctx.send("100 :euro: 얻었습니다!")
